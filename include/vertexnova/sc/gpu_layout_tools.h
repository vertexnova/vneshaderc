...
[[nodiscard]] bool loadGpuLayoutRegistry(const std::filesystem::path& path, GpuLayoutRegistry& out,
                                         std::string& error);
...
[[nodiscard]] bool mergeGpuLayoutRegistries(const std::vector<std::filesystem::path>& registry_paths,
                                              ...
                                              GpuLayoutRegistry& out, std::string& error);

/// Compare reflected UBO sizes against @p registry. Returns false and sets @p error on mismatch.
[[nodiscard]] bool validateGpuLayouts(const ShaderArtifact& artifact, const GpuLayoutRegistry& registry,
                                      std::string& error);

/// Emit composed GLSL: compose #includes + reflected layout(...) declarations.
[[nodiscard]] std::string buildBindingDeclsGlsl(const ShaderArtifact& artifact,
                                                const EmitBindingDeclOptions& options);
...
[[nodiscard]] bool writeBindingDeclsFile(const std::filesystem::path& output_path, const std::string& contents,
                                         std::string& error);
